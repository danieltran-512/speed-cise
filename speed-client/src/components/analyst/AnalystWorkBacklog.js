...
export const AnalystWorkBacklog = () => {
  ...
  useEffect(() => {
    async function fetchAssignedWork() {
      await axios
        .get(
          `https://cise-speed-2022.herokuapp.com/articles/getArticlesForAnalyst/${user.id}`,
          {
  ...
  const analyse = async (article) => {
    navigate("/analyse/" + article.id, {
      state: {
        article: article,
      },
    });
      ...
      temp.push(
        <>
          <p>
            ...
            <button onClick={() => analyse(article)}>Analyse</button>
          </p>
        </>
      );
    });
