...
// TODO update evidence results as well
export const Analyse = () => {
  ...
  const analyse = async (id, status) => {
    ...
    await axios
      ...
      .then((res) => {
        setUI(<h1>This article has successfully analysed</h1>);
  ...
  const generateUI = (article) => {
    ...
    temp.push(
      <>
        <p>
          {article.title}
          <button onClick={() => analyse(article.id, "analysed")}>
            Approve
          </button>
          <button onClick={() => analyse(article.id, "rejected")}>
            Reject
          </button>
        </p>
      </>
  ...
  const navigateWorkBacklog = () => {
    navigate("/analystbacklog");
  };
  return (
    <>
      <h1>Analyse Article</h1>
